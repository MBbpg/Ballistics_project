...
void calculations(double target, double wind, double v0, double m, double A)
{
    ...
    for (int i = 1; i <= 90; i += 1)
    {
        ...
        while (tland == 0)
        {
            ...
            else if (ttop != 0 && hterm == 0) /*If this is true, we have reached the top before, but we haven't reached terminal velocity yet*/
            {
                ...
                vy += acc * dt;
                h2 -= vy * dt;
                if (acc < 0.00001)/*If we reach an acceleration of 0.00001, we are safe to say that we have reached a point where further acceleration is negligible*/
                {
                    hterm = h2;
                }
            }
...
int main()
{
    ...
    while (end == 1)
    {
        ...
        case 6: end = sure(); /**Depending on the users choice, either the program ends, and the calculations begin, or it just returns to the menu**/
            if (end == 0)
                calculations(target, wind, v0, m, A);
            break;
